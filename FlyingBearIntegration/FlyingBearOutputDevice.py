...
class FlyingBearOutputDevicePlugin(OutputDevicePlugin): #We need to be an OutputDevicePlugin for the plug-in system.
    ##  Called upon launch.
    #
    #   You can use this to make a connection to the device or service, and
    #   register the output device to be displayed to the user.
    def start(self):
        self.getOutputDeviceManager().addOutputDevice(FlyingBearOutputDevice()) #Since this class is also an output device, we can just register ourselves.
    ...
    def stop(self):
        self.getOutputDeviceManager().removeOutputDevice("flyingbear_output_device") #Remove all devices that were added. In this case it's only one.

class FlyingBearOutputDevice(OutputDevice): #We need an actual device to do the writing.
    def __init__(self):
        super().__init__("flyingbear_output_device") #Give an ID which is used to refer to the output device.

        #Optionally set some metadata.
        self.setName("FlyingBear Output Device") #Human-readable name (you may want to internationalise this). Gets put in messages and such.
        self.setShortDescription(catalog.i18nc("@action:button", "Send To Printer")) #This is put on the save button.
        self.setDescription(catalog.i18nc("@item:inlistbox", "Send to FlyingBeat printer via WIFI"))
